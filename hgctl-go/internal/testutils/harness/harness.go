...
func (h *TestHarness) Setup() error {
	...
	// Set HGCTL_CONFIG_DIR for this test
	os.Setenv("HGCTL_CONFIG_DIR", h.ConfigDir)
...
func (h *TestHarness) ExecuteCLIWithKeystore(keystoreName string, args ...string) (*CLIResult, error) {
	...
	// Save and set HGCTL_CONFIG_DIR
	originalEnv["HGCTL_CONFIG_DIR"] = os.Getenv("HGCTL_CONFIG_DIR")
	os.Setenv("HGCTL_CONFIG_DIR", h.ConfigDir)
	...
	if keystoreName != "" {
		...
		// Add keystore-specific arguments
		args = append([]string{
	...
	// Add context to args
	if h.ContextName != "" {
		args = append([]string{"--context", h.ContextName}, args...)
	}
...
func (h *TestHarness) createTestContext() error {
	...
	if err := h.app.Run([]string{"hgctl", "context", "create", "--name", h.ContextName, "--use"}); err != nil {
		return fmt.Errorf("failed to create context: %w", err)
	}

	// Set each configuration value using separate context set calls
	contextSetCalls := [][]string{
		{"hgctl", "context", "set", "--rpc-url", h.ChainConfig.L1RPC},
		...
		{"hgctl", "context", "set", "--avs-address", h.ChainConfig.AVSTaskRegistrarAddress},
	...
	h.logger.Info("Created test context config", zap.String("context", h.ContextName))
