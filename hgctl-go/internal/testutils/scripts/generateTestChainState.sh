...
create_keystore() {
    ...
    rm -rf "$HOME/.hgctl/$context/keystores/$name" 2>/dev/null || true
    rm -rf "$HOME/.config/hgctl/$context/keystores/$name" 2>/dev/null || true
...
create_keystore "test" "aggregator" "bn254" "$AGGREGATOR_BN254_PK" "$AGGREGATOR_PASSWORD"

echo "Creating BN254 keystore for executor..."
create_keystore "test" "executor" "bn254" "$EXECUTOR_BN254_PK" "$EXECUTOR_PASSWORD"

# Create ECDSA keystores for transaction signing
echo "Creating ECDSA keystore for aggregator..."
create_keystore "test" "aggregator-ecdsa" "ecdsa" "$AGGREGATOR_ECDSA_PK" "$AGGREGATOR_PASSWORD"

echo "Creating ECDSA keystore for executor..."
create_keystore "test" "executor-ecdsa" "ecdsa" "$EXECUTOR_ECDSA_PK" "$EXECUTOR_PASSWORD"
...
ls -la "$HOME/.config/hgctl/test/keystores/" 2>/dev/null || echo "No keystores in ~/.config/hgctl/test/keystores/"
