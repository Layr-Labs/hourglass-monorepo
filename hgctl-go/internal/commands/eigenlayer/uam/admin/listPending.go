...
func ListPendingCommand() *cli.Command {
	return &cli.Command{
		Name:  "list-pending",
		Usage: "List all pending admins for an account",
		Description: `List all pending admins for an account in the PermissionController.

Flags:
--account-address  The account address to list pending admins for (defaults to operator address from context)

Usage:
  hgctl eigenlayer user admin list-pending  # Uses operator address from context
  hgctl eigenlayer user admin list-pending --account-address 0x1234...`,
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:     "account-address",
				Usage:    "Account address to list pending admins for (defaults to operator address from context)",
				Required: false,
			},
		},
...
func listPendingAction(c *cli.Context) error {
	...
	log.Debug("Fetching pending admins",
		zap.String("accountAddress", accountAddress.Hex()),
	)

	pendingAdmins, err := contractClient.GetPendingAdmins(c.Context, accountAddress)
	if err != nil {
		log.Error("Failed to get pending admins", zap.Error(err))
		return fmt.Errorf("failed to get pending admins: %w", err)
	}
