...
// ContractClientKey is the context key for storing the contract client
const ContractClientKey = "contractClient"
...
func ContractBeforeFunc(c *cli.Context) error {
	...
	// Get current context
	currentCtx, ok := c.Context.Value("currentContext").(*config.Context)
	...
	if rpcURL == "" {
		log.Debug("RPC URL not configured, skipping contract client initialization")
		return nil
	}
	...
	if privateKey == "" {
		log.Debug("Private key not provided, contract client will be read-only")
	}
	...
	// Store in context
	c.Context = context.WithValue(c.Context, ContractClientKey, contractClient)
...
func GetContractClient(c *cli.Context) (*client.ContractClient, error) {
	contractClient, ok := c.Context.Value(ContractClientKey).(*client.ContractClient)
	if !ok || contractClient == nil {
		return nil, fmt.Errorf("contract client not initialized. Please configure AVS address, operator address, and RPC URL")
	}
...
func CleanupContractClient(c *cli.Context) error {
	if contractClient, ok := c.Context.Value(ContractClientKey).(*client.ContractClient); ok && contractClient != nil {
