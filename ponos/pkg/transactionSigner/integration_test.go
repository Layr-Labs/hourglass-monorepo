...
// TestFactoryIntegration tests the factory pattern with different signer types
func TestFactoryIntegration(t *testing.T) {
	logger := zaptest.NewLogger(t)

	// Test private key signer creation via factory
	privateKeyConfig := &SignerConfig{
		Type:       "private_key",
		PrivateKey: "0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d",
	}

	// We can't easily test CreateSigner without a real ethClient, so we test the components
	signingContext := &SigningContext{
		ethClient: nil,
		logger:    logger,
		chainID:   big.NewInt(31337),
	...
	privateSigner, err := NewPrivateKeySigner(privateKeyConfig.PrivateKey, signingContext)
	require.NoError(t, err)
	assert.NotNil(t, privateSigner)

	// Test web3signer creation via factory components
	...
	// Test that both implement the TransactionSigner interface
	var _ TransactionSigner = privateSigner
	var _ TransactionSigner = web3Signer

	t.Logf("Successfully created both signer types via factory components")
}
