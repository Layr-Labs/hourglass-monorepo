...
func TestSignResult_ECDSA(t *testing.T) {
	...
	// Test
	signature, digest, err := e.signResult(task, result)
	...
	// Verify signature contains executor address
	sigData := e.createTaskResultSignatureData(task, result)
	assert.Equal(t, executorAddress, sigData.OperatorAddress, "Signature data should contain executor address")
	assert.Equal(t, task.TaskID, sigData.TaskId, "Signature data should contain task ID")
	assert.Equal(t, result.Result, sigData.Output, "Signature data should contain result output")
...
func TestSignResult_BindsToExecutor(t *testing.T) {
	...
	sig1, digest1, err := e1.signResult(task, result)
	require.NoError(t, err)
	
	sig2, digest2, err := e2.signResult(task, result)
	require.NoError(t, err)
	
	// Assert
	assert.NotEqual(t, sig1, sig2, "Different executors should produce different signatures")
	assert.NotEqual(t, digest1, digest2, "Different executors should produce different digests")
	
	// Verify each signature contains correct executor address
	sigData1 := e1.createTaskResultSignatureData(task, result)
	assert.Equal(t, executor1Address, sigData1.OperatorAddress)
	
	sigData2 := e2.createTaskResultSignatureData(task, result)
	assert.Equal(t, executor2Address, sigData2.OperatorAddress)
}
...
func TestSignResult_IncludesAllFields(t *testing.T) {
	...
	result := &performerTask.PerformerTaskResult{
		TaskID: task.TaskID,
		Result: []byte("complex output with unicode 你好世界 and emojis 🚀"),
	}
	...
	// Verify all fields are included in signature data
	sigData := e.createTaskResultSignatureData(task, result)
	assert.Equal(t, task.TaskID, sigData.TaskId, "Task ID should be included")
	assert.Equal(t, avsAddress, sigData.AvsAddress, "AVS address should be included")
	assert.Equal(t, executorAddress, sigData.OperatorAddress, "Executor address should be included")
	assert.Equal(t, uint32(2), sigData.OperatorSetId, "Operator set ID should be included")
	assert.Equal(t, result.Result, sigData.Output, "Result output should be included")
...
func TestSignResult_MissingSignerError(t *testing.T) {
	...
	assert.Nil(t, signature, "Signature should be nil on error")
	assert.Nil(t, digest, "Digest should be nil on error")
